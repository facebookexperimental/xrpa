...
import { XredImageViewerInterface } from "../js/ImageViewerInterface";

const apidir = path.join(__dirname, "..", "ImageViewer");

const ImageViewerModule = XrpaPythonStandalone("ImageViewer", apidir, () => {
  setProgramInterface(XredImageViewerInterface);
});

export async function doCodegen() {
  const filesToWrite = ImageViewerModule.doCodeGen();
