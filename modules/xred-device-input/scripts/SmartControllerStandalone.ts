...
export async function doCodegen() {
  await SmartControllerStandalone.doCodeGen().finalize(SmartControllerStandalone.manifestFilename);
}

export async function doBuild() {
  const { runProcess } = await import("@xrpa/xrpa-file-utils");
  const mode = SmartControllerStandalone.getBuckMode("debug");
  const target = await SmartControllerStandalone.getStandaloneTarget();
...
export async function doRun() {
  ...
  await buckRun({
    mode: SmartControllerStandalone.getBuckMode("debug"),
    target: await SmartControllerStandalone.getStandaloneTarget(),
    resourceFilenames: (SmartControllerStandalone as any).resourceFilenames || [],
  });
}
...
export async function runStandalone() {
  await SmartControllerStandalone.buckRunDebug();
}
