...
import { XredSpeakerIdentificationInterface } from "../js/SpeakerIdentificationInterface";

const apidir = path.join(__dirname, "..", "SpeakerID");

const SpeakerIdentificationModule = XrpaPythonStandalone("SpeakerID", apidir, () => {
  setProgramInterface(XredSpeakerIdentificationInterface);
});

export async function doCodegen() {
  const filesToWrite = SpeakerIdentificationModule.doCodeGen();
