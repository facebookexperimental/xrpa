...
class QuestModule : public Xrpa::XrpaModule {
 public:
  QuestModule(std::shared_ptr<Xrpa::TransportStream> TrackingInboundTransport, std::shared_ptr<Xrpa::TransportStream> TrackingOutboundTransport) {
    trackingDataStore = std::make_shared<TrackingDataStore::TrackingDataStore>(TrackingInboundTransport, TrackingOutboundTransport);
  }

  virtual ~QuestModule() override {
    shutdown();
  }
