...
export async function doCodegen() {
  await OptiTrackStandalone.doCodeGen().finalize(OptiTrackStandalone.manifestFilename);
}

export async function doBuild() {
  const { runProcess } = await import("@xrpa/xrpa-file-utils");
  const mode = OptiTrackStandalone.getBuckMode("debug");
  const target = await OptiTrackStandalone.getStandaloneTarget();
...
export async function doRun() {
  ...
  await buckRun({
    mode: OptiTrackStandalone.getBuckMode("debug"),
    target: await OptiTrackStandalone.getStandaloneTarget(),
    resourceFilenames: (OptiTrackStandalone as any).resourceFilenames || [],
  });
}
...
export async function runStandalone() {
  await OptiTrackStandalone.buckRunDebug();
}
