...
import { XredTextToSpeechInterface } from "../js/TextToSpeechInterface";

const apidir = path.join(__dirname, "..", "TTS");

const TextToSpeechModule = XrpaPythonStandalone("TextToSpeech", apidir, () => {
  setProgramInterface(XredTextToSpeechInterface);
});

export async function doCodegen() {
  const filesToWrite = TextToSpeechModule.doCodeGen();
