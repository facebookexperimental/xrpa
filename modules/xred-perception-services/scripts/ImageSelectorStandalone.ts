...
export async function doCodegen() {
  await ImageSelectorStandalone.doCodeGen().finalize(ImageSelectorStandalone.manifestFilename);
}

export async function doBuild() {
  const { runProcess } = await import("@xrpa/xrpa-file-utils");
  const mode = ImageSelectorStandalone.getBuckMode("debug");
  const target = await ImageSelectorStandalone.getStandaloneTarget();
...
export async function doRun() {
  ...
  await buckRun({
    mode: ImageSelectorStandalone.getBuckMode("debug"),
    target: await ImageSelectorStandalone.getStandaloneTarget(),
    resourceFilenames: (ImageSelectorStandalone as any).resourceFilenames || [],
  });
}
...
export async function runStandalone() {
  await ImageSelectorStandalone.buckRunDebug();
}
