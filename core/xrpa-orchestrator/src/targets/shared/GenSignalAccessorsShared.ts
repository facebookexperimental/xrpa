...
import { CollectionTypeDefinition, SignalDataTypeDefinition, typeIsSignalData } from "../../shared/TypeDefinition";
import { CodeLiteralValue, ConstructValue } from "../../shared/TypeValue";
import { TargetCodeGenImpl } from "../../shared/TargetCodeGen";

export function genSignalDispatchBody(params: {
  ...
  for (const fieldName in typeFields) {
    ...
    lines.push(
      ...params.codegen.ifEquals(params.codegen.identifierName("messageType"), `${msgType}`,
        params.codegen.genMessageDispatch({
...
function genOnSignalAccessor(classSpec: ClassSpec, params: {
  codegen: TargetCodeGenImpl,
  fieldName: string,
  ...
  classSpec.members.push({
    name: signalHandler,
    type: params.codegen.genSharedPointer(inboundSignalDataInterface, classSpec.includes),
    initialValue: new CodeLiteralValue(params.codegen, params.codegen.getNullValue()),
    visibility: "private",
  });
}

function genSendSignalAccessor(classSpec: ClassSpec, params: {
  codegen: TargetCodeGenImpl,
  ...
  const localFieldVar = params.codegen.privateMember(`local${upperFirst(params.fieldName)}`);
  ...
  classSpec.methods.push({
    name: params.name ?? `set${upperFirst(params.fieldName)}Callback`,
  ...
  classSpec.methods.push({
    name: params.name ?? `set${upperFirst(params.fieldName)}RingBuffer`,
    decorations: params.decorations,
    templateParams: canInferSampleType ? ["SampleType"] : undefined,
    whereClauses: canInferSampleType ? ["SampleType : unmanaged"] : undefined,
    parameters: [{
  ...
  classSpec.methods.push({
    name: params.name ?? `set${upperFirst(params.fieldName)}Forwarder`,
    decorations: params.decorations,
    templateParams: canInferSampleType ? ["SampleType"] : undefined,
    whereClauses: canInferSampleType ? ["SampleType : unmanaged"] : undefined,
    parameters: [{
    ...
    body: () => {
      return [
        params.codegen.genDeclaration({
          ...
          varName: "sampleType",
          initialValue: new CodeLiteralValue(params.codegen, params.codegen.genMethodCall("", inferSampleType, canInferSampleType ? [] : [params.codegen.identifierName("sampleTypeName")])),
  ...
  classSpec.members.push({
    ...
    initialValue: new ConstructValue(params.codegen, OutboundSignalData.getLocalType(classSpec.namespace, classSpec.includes), classSpec.namespace),
    visibility: "private",
  });

  const messageType = params.typeDef.getFieldIndex(params.fieldName);

  params.tickLines.push(
    params.codegen.genMethodCall(localFieldMember, "setRecipient", ["id", collection, `${messageType}`]) + params.codegen.STMT_TERM,
    params.codegen.genMethodCall(localFieldMember, "tick", []) + params.codegen.STMT_TERM,
  );
}

export function genSignalFieldAccessors(classSpec: ClassSpec, params: {
  codegen: TargetCodeGenImpl,
  reconcilerDef: InputReconcilerDefinition | OutputReconcilerDefinition,
  proxyObj: string | null,
}): void {
    ...
    classSpec.methods.push({
      ...
      body: [
        params.codegen.genDeclaration({
          ...
          inNamespace: classSpec.namespace,
          varName: "id",
          initialValue: new CodeLiteralValue(params.codegen, getXrpaId),
          includeTerminator: true,
        }),
        ...tickLines,
      ],
