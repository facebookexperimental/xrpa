...
import { HEADER } from "../cpp/CppCodeGenImpl";
...
function writeUPlugin(fileWriter: FileWriter, pluginRoot: string, pluginName: string, pluginDeps: [string, string][]) {
  const lines = [
    `{`,
    `  "FileVersion": 3,`,
    `  "Version": 1,`,
    ...
    `  ],`,
    `  "Plugins": [`,
    ...indent(2, removeLastTrailingComma(pluginDeps.filter(entry => entry[0] !== "").map(entry => `{ "Name": "${entry[0]}", "Enabled": true },`))),
    `  ]`,
    `}`,
  ];

  fileWriter.writeFile(path.join(pluginRoot, `${pluginName}.uplugin`), lines);
}

function writeBuildFile(fileWriter: FileWriter, pluginRoot: string, pluginName: string, pluginDeps: [string, string][]) {
  const lines = [
    ...HEADER,
    `using System.IO;`,
    ...
    `        "Engine",`,
    `        "CoreUObject",`,
    ...indent(4, pluginDeps.map(entry => `"${entry[1]}",`)),
    `      }`,
    `    );`,
    ``,
  ...
  fileWriter.writeFile(path.join(pluginRoot, "Source", pluginName, `${pluginName}.Build.cs`), lines);
}

function writePluginModuleCpp(fileWriter: FileWriter, outDir: string, pluginName: string) {
  const lines = [
    ...HEADER,
  ...
  fileWriter.writeFile(path.join(outDir, `${pluginName}.cpp`), lines);
}

function writePluginModuleHeader(fileWriter: FileWriter, outDir: string, pluginName: string) {
  const lines = [
    ...HEADER,
    `#pragma once`,
    ``,
  ...
  fileWriter.writeFile(path.join(outDir, `${pluginName}.h`), lines);
}

export function genPlugin(fileWriter: FileWriter, pluginRoot: string, pluginName: string, pluginDeps: Array<[string, string]> = []) {
  const resourcesDir = path.join(pluginRoot, "Resources");
  const privateSrcDir = path.join(pluginRoot, "Source", pluginName, "Private");
  const publicSrcDir = path.join(pluginRoot, "Source", pluginName, "Public");

  fileWriter.writeFileBase64(path.join(resourcesDir, "Icon128.png"), Icon128)
