...
import { PythonIncludeAggregator, HEADER, genClassDefinition, identifierName } from "./PythonCodeGenImpl";
...
export function genDataflowProgram(
  ...
  const filename = `${identifierName(programDef.interfaceName)}.py`;

  const classSpec = genDataflowProgramClassSpec(
    ctx,
    PythonCodeGenImpl,
    programDef,
    new PythonIncludeAggregator(),
  );
