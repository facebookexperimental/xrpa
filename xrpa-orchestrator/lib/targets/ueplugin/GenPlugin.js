...
function writeUPlugin(fileWriter, pluginRoot, pluginName, pluginDeps) {
    const lines = [
        ...
        `  ],`,
        `  "Plugins": [`,
        ...(0, Helpers_1.indent)(2, (0, Helpers_1.removeLastTrailingComma)(pluginDeps.filter(entry => entry[0] !== "").map(entry => `{ "Name": "${entry[0]}", "Enabled": true },`))),
        `  ]`,
        `}`,
    ];
...
function writeBuildFile(fileWriter, pluginRoot, pluginName, pluginDeps) {
    const lines = [
        ...
        `        "Engine",`,
        `        "CoreUObject",`,
        ...(0, Helpers_1.indent)(4, pluginDeps.map(entry => `"${entry[1]}",`)),
        `      }`,
        `    );`,
        ``,
