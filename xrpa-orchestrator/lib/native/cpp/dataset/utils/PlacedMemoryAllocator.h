...
struct PlacedMemoryAllocator {
  ...
  MemoryAccessor alloc(int32_t numBytes) {
    ...
    if (foundSize - sizeNeeded > PMA_SPLIT_THRESHOLD) {
      ...
      PMAFreeNode* splitNode = getAt<PMAFreeNode>(splitOffset);
    ...
    return MemoryAccessor(
        getAt<uint8_t>(0), foundOffset + sizeof(PMAAllocNode), sizeNeeded - sizeof(PMAAllocNode));
  }
  ...
  MemoryAccessor get(int32_t offset) {
    ...
    return MemoryAccessor(getAt<uint8_t>(0), offset, size - sizeof(PMAAllocNode));
  }
  ...
  const void* offsetToPtr(int32_t offset) const {
    if (offset < sizeof(PMAAllocNode)) {
      return nullptr;
    }
