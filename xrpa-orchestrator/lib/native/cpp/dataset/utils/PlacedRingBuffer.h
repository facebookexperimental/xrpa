...
struct PlacedRingBuffer {
  ...
  MemoryAccessor getAt(int32_t index) {
    if (index >= count) {
  ...
  MemoryAccessor getByID(int32_t id) {
  ...
  int32_t getIndexForID(int32_t id) const {
    if (!count || id < startID) {
      return 0;
    }
    return id - startID;
 ...
 private:
  ...
  int32_t getElementSize(int32_t offset) const {
    xrpaDebugBoundsAssert(offset, 4, 0, poolSize);
  ...
  MemoryAccessor getElementAccessor(int32_t offset) {
    int32_t numBytes = getElementSize(offset);
  ...
  int32_t getOffsetForIndex(int32_t index) const {
    int32_t offset = startOffset;
  ...
  int32_t findFreeOffset(int32_t sizeNeeded) {
    if (count == 0) {
      xrpaDebugAssert(startOffset == 0);
      return startOffset;
    }

    int32_t lastElemOffset = getOffsetForIndex(count - 1);
